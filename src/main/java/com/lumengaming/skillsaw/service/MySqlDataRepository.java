...
public class MySqlDataRepository implements IDataRepository {
  ...
  private boolean connect() {
    try {
      ...
      } else if (this.connection != null && !this.connection.isValid(5)) {
        try {
...
//        return false;
  ...
  /**
   * Maximum of 1000 results. If more than that many are returned, something probably went wrong
   * with the query.
   *
   * @param username
   * @return
  ...
  /**
   * Expects that RS has been primed before calling. "rs.next()". Returns null on failure, also
   * throws exception into console.
   */
