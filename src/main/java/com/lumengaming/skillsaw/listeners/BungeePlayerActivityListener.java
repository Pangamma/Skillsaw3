...
public class BungeePlayerActivityListener implements Listener {
  ...
  public void onLogin(final PostLoginEvent e) {
    ...
    String host = e.getPlayer().getPendingConnection().getVirtualHost().getHostName();
    final Optional<ForcedHostOption> forcedHost = Options.Get().ForcedHosts.stream()
            .filter(x -> x.Host.equalsIgnoreCase(host)
            && x.ChatChannelOnJoin != null
            && x.ChatChannelOnJoin.length() > 0
            ).findFirst();

    plugin.getDataService().loginUser(new BungeePlayer(e.getPlayer()), (u) -> {
      if (forcedHost.isPresent()) {
        u.setSpeakingChannel(forcedHost.get().ChatChannelOnJoin);
      }
      ...
      synchronized (this.userWelcomes) {
        if (this.userWelcomes.containsKey(p.getUniqueId())) {
          ...
          if (user != null) {
            ...
            if (bp.getIpv4().equals("173.249.30.10")) {
              return;
            }
