...
public class SharedUtility {
  ...
  /**
   * @param args The number of leading arguments to skip.
   * @param skip
   * @return
   */
  public static String[] getSubArgs(String[] args, int skip) {
    String[] subarray = new String[args.length - skip];
    System.arraycopy(args, skip, subarray, 0, subarray.length);
    return subarray;
  }
  ...
  /**
   * Expects input to be pre-prepared to § color code format. Goes through the input string and
   * removes all the color codes as needed. Useful for doing stuff like checking if user has
   * permissions to use certain color codes. Or maybe not for checking. But certainly for doing
   * actions based on it. *
   */
