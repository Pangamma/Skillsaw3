...
import com.lumengaming.skillsaw.commands.admin.LockoutCommand;
...
public class BungeeMain extends Plugin implements ISkillsaw {
  ...
  private LockoutCommand lockoutCommand;
  ...
  public void onEnable() {
    ...
    this.lockoutCommand = new LockoutCommand(this);
    this.getProxy().getPluginManager().registerCommand(this, lockoutCommand);
    this.getProxy().getPluginManager().registerListener(this, lockoutCommand);
    this.lockoutCommand.onEnable();
...
//            this.getProxy().getPluginManager().registerCommand(this, new MuteListCommand(this));
  ...
  public void onDisable() {
    ...
    this.lockoutCommand.onDisable();
    this.lockoutCommand = null;
...
//        this.getProxy().getPlayer("Pangamma").sendMessage(legacyText);
  ...
  public void printHelp(BungeePlayer cs) {
    ...
    ArrayList<CommandSyntax> syntaxList = BungeeCommand.getSyntaxList(false);
    for (CommandSyntax syntax : syntaxList) {
      if (syntax.canView(cs)) {
        BaseComponent[] toSend = syntax.getSyntax(cs);
...
//    }
  ...
  public LockoutCommand getLockoutCommand() {
    return this.lockoutCommand;
  }
}
