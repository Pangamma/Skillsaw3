...
public class TitleCommand extends BungeeCommand {
  ...
  public Iterable<String> onTabCompleteBeforeFiltering(CommandSender cs, String[] args) {
    ...
    User issuer = this.ds.getUser(p.getUniqueId());
    Set<String> titles = issuer.getAllTitles().stream().map(x -> CText.stripColors(x.getLongTitle())).collect(toSet());
    ...
    switch (args.length) {
      ...
      case 2:
        switch (args[0].toLowerCase()) {
          ...
          case "+":
          case "add":
            set.add("<short/long title>");
            break;
          default:
            set.add("-");
            set.add("+");
            set.add("list");
            User target = this.ds.getUserBestOnlineMatch(args[0]);
            if (target != null) {
              Set<String> targetTitles = target.getAllTitles().stream().map(x -> CText.stripColors(x.getLongTitle())).collect(toSet());
              set.addAll(targetTitles);
            }
            break;
        }
      default:
...
//            return;
//        }
    User issuer = cs.isPlayer() ? this.ds.getUser(cs.getUniqueId()) : null;
//                  1 = list                                 /title list
...
//                  4 = add for other users          /title [user] add [short] [long]
  ...
  private void doAddTitle(BungeePlayer cs, User target, String shortTitle, String longTitle) {
    ...
    if (isSelf) {
      ...
      target.sendMessage("§aYou have gained the '" + lTitle + "§a' title!");
    }
  }
}
