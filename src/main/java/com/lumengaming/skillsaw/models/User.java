...
//    protected IPlayer p;
  ...
  /**
   * Use only for creating a new default user object when one can't be pulled from the repository.
   *
   * @param plugin
   * @param p
   */
  ...
  protected User(User orig) {
    ...
    this.totalMinutesPlayed = orig.totalMinutesPlayed;
    this.p = orig.p;
  ...
  public User(ISkillsaw p_plugin, UUID p_uuid, String p_username, String p_displayName, long p_lastPlayed,
          ...
          String p_speakingChannel, CopyOnWriteArraySet<String> p_stickyChannels, int p_totalMinutesPlayed,
    ...
    this.totalMinutesPlayed = p_totalMinutesPlayed;
  ...
  /**
   * @return @deprecated Replace with "getCurrentTitle"
   */
  @Deprecated
  ...
  /**
   * Shows the display name as the display text. On hover, show username. On click, suggest /msg
   * username
   *
   * @return
   */
  ...
  /**
   * does not check to see if values are valid, does not log anything. All it does is increment the
   * value.*
   */
  ...
  /**
   * Passed in value is 5? Calculates maximum amount of rep that you can have at level 4 before
   * being promoted to level 5.*
   */
  ...
  /**
   * Adds rep without checking the amount that the sender can send, or the amount the target can
   * receive. *
   */
  ...
  /**
   * returns the number of rep points a that are required to complete a 100% transition from one
   * level to the next level. Make sure any rep amount changes to this player do not surpass this
   * limit. This prevents people from leveling multiple times from one transaction.*
   */
  ...
  /**
   * returns the number of rep points a that are required to complete a 100% transition from one
   * level to a lower level. Make sure any rep amount changes to this player do not surpass this
   * limit. This prevents people nuking each other in single transactions.*
   */
...
//		ArrayList<SkillType> skillTypes = plugin.getConfigHandler().getSkillTypes();
  ...
  /**
   * Returns the channel the player is currently speaking in. If opted out, this will instead
   * return, ""
   */
  ...
  public void setTotalMinutesPlayed(int activityTotalMinutes) {
    this.totalMinutesPlayed = activityTotalMinutes;
  }
