...
public class DataService {
  ...
  /**
   * Save is asynchronous. If user is not writable, an illegal state exception is thrown. So
   * basically, make sure it is not being called before this is initialized.
   *
   * @param user
   */
  ...
  /**
   * Should probably run this asynchronously. Use this only when making users for the first time. *
   */
  ...
  /**
   * The list is read only. The users can be edited... in theory. Try to avoid editing any users
   * within this list if you can. Concurrency is a tricky thing that creates hard to find bugs if
   * you aren't careful. To be safe, avoid writing at all to the users from this method.
   *
   * @return
   */
  ...
  /**
   * Sends message to all ChatPlayers listening to the channel. Respects the ignored sender lists
   * of each chat player. *
   */
  ...
  /**
   * Must be called from a synchronous context. Creates user if not exist. Otherwise updates
   * username, ipv4, and lastPlayed time.
   *
   * @param p *
   */
  ...
  /**
   * Saves the user and removes it from the online user list. Save operation happens
   * asynchronously. Don't use this method for closing the plugin. Use save all users instead.
   *
   * @param p
   */
  ...
  /**
   * Only checks through currently online users for the data. Null if not found. *
   */
  ...
  /**
   * Attempts to resolve the partial name into a full name of an online player. If found, the User
   * is returned. Otherwise null is returned. Only checks results from online users. Will make no
   * calls to file system for the data.
   *
  ...
  /**
   * All users from the database where is_staff = 1 will be returned. Ordered by most recent login.
   *
   * @param uuid
   * @param useCache
   * @param callback
  ...
  /**
   * All users from the database where is_instructor = 1 will be returned. Ordered by most recent
   * login.
   *
   * @param uuid
   * @param useCache
  ...
  /**
   * IO heavy operation. Loads the user from the database. Run this asynchronously. if useCache is
   * true, users can be loaded from the online users hashmap before being taken from the database.
   *
   * @param uuid
   * @param useCache
   * @param callback
  ...
  public void getOfflineUsersByIP(String ipv4, AsyncManyCallback<User> callback) {
    if (ipv4 == null || ipv4.isEmpty()) {
      callback.doCallback(new ArrayList<>());
      return;
    }
    plugin.runTaskAsynchronously(() -> {
  ...
  /** Also increments user's activity total minutes as well */
  public void logActivity(UUID playerUUID, String serverName, boolean afk) {
    User u = this.getUser(playerUUID);
    if (u != null) {
      u.setTotalMinutesPlayed(u.getTotalMinutesPlayed() + 12);
    }
    plugin.runTaskAsynchronously(() -> {
